...
function Swap(props) {
  ...
  async function fetchDexSwap() {
    const responseData = await axios.get(`http://localhost:3001/allowance`, {
      params: { tokenAddress: tokenOne.address, walletAddress: address },
    });

    if (responseData.data.allowance === "0") {
      const approve = await axios.get(`http://localhost:3001/transaction`, {
    ...
    const tx = await axios.get(`http://localhost:3001/swap`, {
      params: {
